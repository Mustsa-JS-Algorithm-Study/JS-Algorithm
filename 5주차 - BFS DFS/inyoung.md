...
### 접근방식
각 컴퓨터가 연결되어있는지 dfs를 돌리면서 연결이 되어 있지 않은 거 발견하면 네트워크 개수에 +1하면 될 거 같은디?
...
function solution(n, computers) {
    ...
    function dfs(current) {
        ...
        // Explore all the connections for the current node
        for (let i = 0; i < n; i++) {
    ...
    for (let i = 0; i < n; i++) {
        if (!visited[i]) {
            // If not visited, it's a new network
            answer++;
            dfs(i);
        }
    }
...
function solution(n, computers) {
    ...
    function dfs(current){
        ...
        // 이전에 방문한 적 없을 때 
            ...
            // 방문표시해
            visited[current] = true;
            for (item of computers[current]){
                // 연결되어있네? - 계속파고들어서 visit표시해
                if (computers[current][item]===1){
                    dfs(item);
                }
                // 연결안되어있어?  - answer++
                else{
